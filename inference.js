...
export class InferenceEngine {
    /**
     * @param {tf.GraphModel} model - The loaded TFJS model
     * @param {Object} vocab - The loaded vocabulary JSON
     * @param {number} maxLen - The maximum sequence length (default: 40)
     */
    constructor(model, vocab, maxLen = 40) {
        this.model = model;
        this.vocab = vocab;
        this.MAX_LEN = maxLen;

        // Pre-compute inverses and special tokens ONCE
        this.invVocab = Object.fromEntries(Object.entries(vocab).map(([k, v]) => [v, k]));
        this.START_TOKEN = vocab["["];
        this.STOP_TOKEN = vocab["]"];
        this.PAD_TOKEN = vocab["[PAD]"];
    }

    // --- BEAM SEARCH ---
    async decodeBeamBatched(
        word,
        taskToken,
        beam_width = 3,
        temperature = 1.0,
        ...
        // 1. Context now pulls directly from 'this'
        const fusionContext = {
            originalInput: word,
            taskToken,
            vocab: this.vocab,
        ...
        const candidates = tf.tidy(() => {
            let cleanInput = taskToken === "<" ? word.toLowerCase() : word;
            ...
            let encIds = fullText.split("").map((c) => this.vocab[c] || 0).slice(0, this.MAX_LEN);
            while (encIds.length < this.MAX_LEN) encIds.push(this.PAD_TOKEN);

            const encTensor = tf.tensor2d(Array(beam_width).fill(encIds), [beam_width, this.MAX_LEN], "float32");
            let scores = tf.tensor1d([0.0, ...Array(beam_width - 1).fill(-1e9)]);
            let sequences = tf.fill([beam_width, 1], this.START_TOKEN, "int32");

            let finishedSeqs = [];
            let finishedScores = [];

            // Tracking arrays for LogitsProcessors
            let activeProcessorTracking = Array.from({
                length: beam_width
            }, () => ({}));
            let finishedProcessorTracking = [];

            for (let i = 0; i < this.MAX_LEN - 1; i++) {
                const currLen = sequences.shape[1];
                const padSize = this.MAX_LEN - 1 - currLen;
                ...
                let preds = this.model.execute(inputs);
                ...
                if (finishedSeqs.length >= beam_width) {
                    ...
                    if (bestActiveLogProb > -1e8) {
                        const maxPossibleLP = Math.pow((5 + this.MAX_LEN) / 6, alpha);
            ...
            return allCandidates
                .map((c) => {
                    const cleanIds = c.seq.filter(
                        (id) => id !== this.START_TOKEN && id !== this.STOP_TOKEN && id !== this.PAD_TOKEN,
                    );
                    const text = cleanIds.map((id) => this.invVocab[id]).join("");
    ...
    // --- CYCLIC LOSS ---
    async computeCyclicLossBatch(generatedCandidates, originalInput, backwardTaskToken) {
        return tf.tidy(() => {
            const BATCH_SIZE = generatedCandidates.length;

            // 3. Use this.vocab and this.MAX_LEN internally
            const encIdsBatch = generatedCandidates.map((txt) => {
                let cleanTxt = backwardTaskToken === "<" ? txt.toLowerCase() : txt;
                const encText = backwardTaskToken + cleanTxt;
                let ids = encText.split("").map((c) => this.vocab[c] || 0).slice(0, this.MAX_LEN);
                while (ids.length < this.MAX_LEN) ids.push(this.PAD_TOKEN);
                return ids;
            });

            const encTensor = tf.tensor2d(encIdsBatch, [BATCH_SIZE, this.MAX_LEN], "float32");

            let targetTxt = backwardTaskToken === ">" ? originalInput.toLowerCase() : originalInput;
            const fullTargetText = "[" + targetTxt + "]";
            let allTargetIds = fullTargetText.split("").map((c) => this.vocab[c] || 0);

            let decInputIds = allTargetIds.slice(0, this.MAX_LEN - 1);
            while (decInputIds.length < this.MAX_LEN - 1) decInputIds.push(this.PAD_TOKEN);

            const decInputSingle = tf.tensor2d([decInputIds], [1, this.MAX_LEN - 1], "float32");
            const decInputBatch = decInputSingle.tile([BATCH_SIZE, 1]);

            // Execute on this.model
            let preds = this.model.execute({
                enc_in: encTensor,
                dec_in: decInputBatch
            });
            if (Array.isArray(preds)) preds = preds[0];

            // --- 4. Compute Loss ---
            const results = [];
            const limit = Math.min(allTargetIds.length - 1, this.MAX_LEN - 1);
    ...
    async computeCyclicLoss(generatedIpa, originalWord, backwardTaskToken) {
        const results = await this.computeCyclicLossBatch([generatedIpa], originalWord, backwardTaskToken);
        return results[0];
    }
}

// TODO: Not thrilled by the circular dependancy here.
// 4. Update the CyclicSequenceScorer to just take the engine
export class CyclicSequenceScorer extends SequenceScorer {
    ...
    async score(candidates, context) {
        ...
        const lossResults = await this.engine.computeCyclicLossBatch(textInputs, context.originalInput, backwardTaskToken);
